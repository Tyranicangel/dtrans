...
class Iters(object):
	...
	def run_thread(self):
		# if(self.lv['condtype']=='rectangular'):
		# 	lvconds=Conductor.objects.filter(width__gte=self.iters['lvwidth']['min']).filter(width__lte=self.iters['lvwidth']['max']).filter(thickness__lte=self.iters['lvthk']['max']).filter(thickness__gte=self.iters['lvthk']['min'])
		# elif(self.lv['condtype']=='round'):
		# 	lvconds=Circular.objects.filter(width__gte=self.iters['lvwidth']['min']).filter(width__lte=self.iters['lvwidth']['max'])
		# else:
		# 	lvconds=[]
		# if(self.hv['condtype']=='rectangular'):
		# 	hvconds=Conductor.objects.filter(width__gte=self.iters['hvwidth']['min']).filter(width__lte=self.iters['hvwidth']['max']).filter(thickness__lte=self.iters['hvthk']['max']).filter(thickness__gte=self.iters['hvthk']['min'])
		# elif(self.hv['condtype']=='round'):
		# 	hvconds=Circular.objects.filter(width__gte=self.iters['hvwidth']['min']).filter(width__lte=self.iters['hvwidth']['max'])
		# else:
		# 	hvconds=[]
		lvconds=[]
		hvconds=[]
		...
		while(cmain<=self.iters['corewidth']['max']):
			# t2=multiprocessing.Process(target=thread_main,args=(cmain,self.basic,self.core,self.tapping,self.lv,self.hv,self.cca,self.cooling,self.tank,self.other,self.check,self.costing,self.iters,q,watt_kg_table,radiator_table,vertical_table,horizontal_table,fins_table,lvconds,hvconds))
			# threads.append(t2)
			# t2.start()
			thread_main(cmain,self.basic,self.core,self.tapping,self.lv,self.hv,self.cca,self.cooling,self.tank,self.other,self.check,self.costing,self.iters,q,watt_kg_table,radiator_table,vertical_table,horizontal_table,fins_table,lvconds,hvconds)
...
def thread_main(cmain,basic,core,tapping,lv,hv,cca,cooling,tank,other,check,costing,iters,q,watt_kg_table,radiator_table,vertical_table,horizontal_table,fins_table,lvconds,hvconds):
	out=[]
	if(lv['condtype']=='round'):
		if(hv['condtype']=='round'):
			if(lv['istype']==True and hv['istype']==True):
				for g in lvconds:
					lvwmain=float(g.width)
					for h in hvconds:
						hvwmain=float(h.width)
							...
							while (lvtmain<=iters['lvturns']['max']):
								...
								while (lvhmain<=iters['lvhvgap']['max']):
									...
									while(lvemain<=iters['lvedge']['max']):
										...
										y=design(basic,core,tapping,lv,hv,cca,cooling,tank,other,check,costing,test,out,watt_kg_table,radiator_table,vertical_table,horizontal_table,fins_table,0,0)
						...
						while(fmain<=iters['flux']['max']):
							...
							while (lvtmain<=iters['lvturns']['max']):
								...
								while (lvhmain<=iters['lvhvgap']['max']):
									...
									while(lvemain<=iters['lvedge']['max']):
										test={'cmain':cmain,'fmain':fmain,'lvtmain':lvtmain,'lvwmain':round(lvwmain,2),'lvkmain':lvkmain,'hvwmain':hvwmain,'hvkmain':hvkmain,'lvhmain':lvhmain,'lvemain':lvemain}
										y=design(basic,core,tapping,lv,hv,cca,cooling,tank,other,check,costing,test,out,watt_kg_table,radiator_table,vertical_table,horizontal_table,fins_table,0,hvarea)
										lvemain=round(lvemain+iters['lvedge']['step'],2)
									lvhmain=round((lvhmain+iters['lvhvgap']['step']),2)
								lvtmain=round((lvtmain+iters['lvturns']['step']),2)
							fmain=round((fmain+iters['flux']['step']),3)
						...
						while(fmain<=iters['flux']['max']):
							...
							while (lvtmain<=iters['lvturns']['max']):
								...
								while (lvhmain<=iters['lvhvgap']['max']):
									...
									while(lvemain<=iters['lvedge']['max']):
										test={'cmain':cmain,'fmain':fmain,'lvtmain':lvtmain,'lvwmain':round(lvwmain,2),'lvkmain':lvkmain,'hvwmain':hvwmain,'hvkmain':hvkmain,'lvhmain':lvhmain,'lvemain':lvemain}
										y=design(basic,core,tapping,lv,hv,cca,cooling,tank,other,check,costing,test,out,watt_kg_table,radiator_table,vertical_table,horizontal_table,fins_table,lvarea,0)
										lvemain=round(lvemain+iters['lvedge']['step'],2)
									lvhmain=round((lvhmain+iters['lvhvgap']['step']),2)
								lvtmain=round((lvtmain+iters['lvturns']['step']),2)
							fmain=round((fmain+iters['flux']['step']),3)
						...
						while(fmain<=iters['flux']['max']):
							...
							while (lvtmain<=iters['lvturns']['max']):
								...
								while (lvhmain<=iters['lvhvgap']['max']):
									...
									while(lvemain<=iters['lvedge']['max']):
										test={'cmain':cmain,'fmain':fmain,'lvtmain':lvtmain,'lvwmain':round(lvwmain,2),'lvkmain':lvkmain,'hvwmain':hvwmain,'hvkmain':hvkmain,'lvhmain':lvhmain,'lvemain':lvemain}
										y=design(basic,core,tapping,lv,hv,cca,cooling,tank,other,check,costing,test,out,watt_kg_table,radiator_table,vertical_table,horizontal_table,fins_table,lvarea,hvarea)
										lvemain=round(lvemain+iters['lvedge']['step'],2)
									lvhmain=round((lvhmain+iters['lvhvgap']['step']),2)
								lvtmain=round((lvtmain+iters['lvturns']['step']),2)
							fmain=round((fmain+iters['flux']['step']),3)
...
def design(basic,core,tapping,lv,hv,cca,cooling,tank,other,check,costing,test,que,watt_kg_table,radiator_table,vertical_table,horizontal_table,fins_table,lvarea,hvarea):
	output={}
	bchanneldata={}
	...
	if(lv['connectiontype']=='Zigzag'):
		connfactor=2/math.sqrt(3)
		zigzagturns=myrounder(test['lvtmain']*connfactor)
		connfactor=zigzagturns/test['lvtmain']
	else:
		connfactor=1
	...
	#get_turn_layer
	get_turn_layer=myrounder((test['lvtmain']*connfactor/lv['nolayers'])*10)/10
	...
	if(core['coretype']=='stacked' and core['num']=='Multi'):
		...
		coredia=myrounder(10*2*math.sqrt((get_core_area*4)/(math.pi*core['uarea']*0.97)))/2
		coredats=[]
		twidth=0
		tarea=0
	...
	#lv_axial_elect
	lv_axial_elect=myrounder(((test['lvwmain']+lv['coveringthk'])*((test['lvtmain']*connfactor/lv['nolayers']*lv['flats'])+lv['transpos']))*100)/100
	...
	if(lv['condtype']=='foil'):
		...
		lvphyef=1
	lv_axial_phy=myceil((test['lvwmain']+lv['coveringthk'])*((((test['lvtmain']*connfactor/lv['nolayers'])+lvphyef)*lv['flats'])+lv['transpos']))
	...
	if(check['matchht']=='yes'):
		...
		#hv_axial_phy
		hv_axial_phy=myceil((test['hvwmain']+hv['coveringthk'])*(((get_hv_turn_layer+hvphyef)*hv['flats'])+hv['transpos']))
		#hv_axial_pack
	...
	#lv_resi_ph
	lv_resi_ph=myrounder((lv_mat_ef*connfactor*lv_lmt*test['lvtmain']/(100000*lv_cond_area))*1000000)/1000000
	...
	#lv_cond_wt
	lv_cond_wt=myrounder((lv_lmt*test['lvtmain']*lv_wt_ef*connfactor*lv_cond_area*wt_pf/1000000)*100)/100
	...
	if(hv['fullthk']<3):
		hv_full_duct_area=(hv_lmt-(10*hv_no_full_duct_runners))*hv_axial_phy*2/1000000*hv['nofull']*0.5
	elif(hv['fullthk']<4):
		if(hv_axial_phy<=350):
			hv_full_duct_area=(hv_lmt-(10*hv_no_full_duct_runners))*hv_axial_phy*2/1000000*hv['nofull']
		else:
			hv_full_duct_area=(hv_lmt-(10*hv_no_full_duct_runners))*hv_axial_phy*2/1000000*hv['nofull']*0.75
	else:
		hv_full_duct_area=(hv_lmt-(10*hv_no_full_duct_runners))*hv_axial_phy*2/1000000*hv['nofull']
	if(hv['fnbthk']<3):
		hv_fnb_duct_area=(hv_fnb_perimeter-(10*hv_no_fnb_duct_runners))*hv_axial_phy*4/1000000*hv['nofnb']*0.5
	elif(hv['fnbthk']<4):
		if(hv_axial_phy<=350):
			hv_fnb_duct_area=(hv_fnb_perimeter-(10*hv_no_fnb_duct_runners))*hv_axial_phy*4/1000000*hv['nofnb']
		else:
			hv_fnb_duct_area=(hv_fnb_perimeter-(10*hv_no_fnb_duct_runners))*hv_axial_phy*4/1000000*hv['nofnb']*0.75
	else:
		hv_fnb_duct_area=(hv_fnb_perimeter-(10*hv_no_fnb_duct_runners))*hv_axial_phy*4/1000000*hv['nofnb']
	...
	hv_outer_duct_cooling_area=0
	if(basic['corelvhv']=='lvstart'):
		hv_full_duct_area_outside=(hv_od_perimeter)*hv_axial_phy/1000000
		if(test['lvhmain']>=8):
	...
	output['hvareadat']=hvarea*hv['radials']*hv['flats']
	output['lvareadat']=lvarea*lv['radials']*lv['flats']
